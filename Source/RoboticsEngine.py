...
class RoboticsEngine(object):
    ...
    def main(self):
        ...
        if (self.useNetworkClient):
            self.networkClientThread = Thread(target =  self.networkClient.establishConnection)
            self.networkClientThread.setDaemon(True)
            self.networkClientThread.start()
        if (self.useDataSyncController):
            self.dataSyncControllerThread = Thread(target = self.dataSyncController.syncLiveData)
